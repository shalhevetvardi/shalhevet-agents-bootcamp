...
**עודכן:** 07-08-2026 | **גרסה חיה:** v0.4.0 (שוחררה לפרודקשן) | **ריפו:** `shalhevet-agents-bootcamp`
...
## הגרסה החיה: v0.4.0 (שוחררה 07-08-2026)
- **מה היא עושה:** אריזה + התקנה אוטומטית + החלפת הישן, **בבטחה**: גיבוי לפני החלפה, אימות אחרי, שחזור-אוטומטי בכישלון (ואז נפילה לידני), 3 בדיקות מבנה לפני אריזה (שורש הפלאגין, מיקום SKILL.md, BOM ב-plugin.json), אריזת Windows דרך `System.IO.Compression` (לא `Compress-Archive`), `robocopy` לפלאגינים כבדים, דילוג בחן אם אין תיקיית Claude Desktop.
...
## קבצים (נתיבים מלאים)
- `skills/rebuild-plugin/SKILL.md` - הלוגיקה (v0.4.0). **מקור-האמת - עורכים כאן.**
- `skills/rebuild-plugin/.claude-plugin/plugin.json` - version 0.4.0.
...
1. **כתיבה לקבצי הרישום של קלוד** (`installed_plugins.json`/`marketplace.json`) - מותרת רק בבטחה: בלי BOM, קריאה-קודם + שימור רשומות, גיבוי+אימות+שחזור, דילוג אם אין Claude Desktop.
