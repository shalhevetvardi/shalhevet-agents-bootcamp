...
6. **v0.4.3 (18-09)** - התלמיד **אלי טל** (eli@rncarmel.co.il) דיווח שההתקנה נדחית ב-Cowork: `description` ב-`plugin.json` היה 541 תווים (עברית+אנגלית כפולה). תיעוד קלוד הרשמי ל-`plugin.json` (manifest-reference.md) לא מצטט 500 כסף קבוע, אלא ממליץ 50-200 תווים; 541 חורג משניהם. קוצר ל-304 תווים (נוסח אחיד עברית+אנגלית קצר, בלי לאבד עובדה מהותית - 4 בדיקות מבנה, רישום מלא, גיבוי+אימות+שחזור, cross-OS), הגרסה עלתה ל-0.4.3, וה-zip נבנה מחדש. **נוסף שומר קבוע** - `scripts/check_plugin_descriptions.py` (עם `--self-test` שמוכיח את עצמו מול המחרוזת בת 541 התווים שבאמת שברה התקנה) - שיתפוס `description` חורג מ-500 תווים בכל `plugin.json` בריפו, ומונע חזרה על התקלה בסקילים עתידיים.
